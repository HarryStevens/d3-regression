...
tape("logarithmic.domain(domain) sets the domain explicitly", function(test) {
  const r = d3.regressionLogarithmic().domain([0, 50]);
  test.deepEqual(r.domain(), [0, 50]);
  test.end();
});

tape("logarithmic(data) calculates the the a, b, and c coefficients, R^2, and returns a line representing the regression", function(test) {
  ...
  const r = d3.regressionLogarithmic()
    .x(d => d.x)
    .y(d => d.y)
    .domain([1e-6, 32])
    (data);
